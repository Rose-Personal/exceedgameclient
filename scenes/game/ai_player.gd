...
func take_boost(game_logic : LocalGame, my_id : Enums.PlayerId, legal_boost_zones : Array, limitation : String) -> BoostAction:
	var me = game_logic._get_player(my_id)
	var opponent = game_logic._get_player(game_logic.get_other_player(my_id))
	# Decide which action makes the most sense to take.
	var possible_actions = get_boost_actions(game_logic, me, opponent, legal_boost_zones, limitation)
	update_ai_state(game_logic, me, opponent)
...
func determine_possible_turn_actions(game_logic : LocalGame, me : LocalGame.Player, opponent : LocalGame.Player):
	...
	possible_actions += get_boost_actions(game_logic, me, opponent, ["hand"], "")
...
func get_boost_actions(game_logic : LocalGame, me : LocalGame.Player, opponent : LocalGame.Player, legal_boost_zones: Array, limitation : String):
	var possible_actions = []
	if "hand" in legal_boost_zones:
		for card in me.hand:
			...
			if does_boost_work(game_logic, me, opponent, card.id):
				possible_actions += get_payment_choices_for_boost(game_logic, me, card)
...
func get_payment_choices_for_boost(game_logic : LocalGame, me : LocalGame.Player, card):
	var possible_payments = []
	...
	if cost > 0:
		...
		for combo in combinations:
			possible_payments.append(BoostAction.new(card.id, combo))
	else:
		possible_payments.append(BoostAction.new(card.id, []))
