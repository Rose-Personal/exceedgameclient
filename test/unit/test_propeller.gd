...
var default_deck = CardDefinitions.get_deck_from_str_id("propeller")
...
func test_propeller_frontside_effect():
	position_players(player1, 5, player2, 6)
	give_gauge(player1, 1)
	execute_strike(player1, player2, "propeller_swoop","standard_normal_assault", [0], [], false, false)
	validate_life(player1, 30, player2, 27)

func test_propeller_exceed_kicker():
	give_gauge(player1, 2)
	position_players(player1, 2, player2, 6)
	game_logic.do_exceed(player1, [player1.gauge[0].id, player1.gauge[1].id])
	assert_true(game_logic.do_choice(player1, 1))
	validate_positions(player1, 4, player2, 6)

func test_propeller_exceed_ability():
...
func test_propeller_saber_lunge():
	give_player_specific_card(player1, "standard_normal_spike", TestCardId3)
	assert_true(game_logic.do_boost(player1, TestCardId3))
	player1.hand = []
	advance_turn(player2)
	position_players(player1, 4, player2, 6)
	execute_strike(player1, player2, "propeller_saberlunge","standard_normal_sweep", [1], [], false, false)
	validate_positions(player1, 9, player2, 6)
	validate_life(player1, 24, player2, 26)
	# Not currently implemented -- attack returns at After instead of Cleanup
	# assert_eq(player1.hand.size(), 0)

func test_propeller_private_reserve():
	var card_db = game_logic.get_card_database()
	give_player_specific_card(player1, "propeller_fullbroadside", TestCardId3)
	give_player_specific_discard(player1, "standard_normal_sweep", TestCardId4)
	var normal_sweep = card_db.get_card(TestCardId4)
	assert_eq(player1.discards.size(), 1)
	assert_true(normal_sweep in player1.discards)
	game_logic.do_boost(player1, TestCardId3)
	game_logic.do_boost(player1, TestCardId4)
	assert_false(normal_sweep in player1.discards)
	assert_true(normal_sweep in player1.continuous_boosts)
